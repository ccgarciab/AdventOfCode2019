...
runAndLink :: Computer -> Computer -> Maybe Computer
runAndLink c1 c2 = do
                   cResult <- run c1
                   result  <- if null $ output cResult then Nothing else Just $ head $ output cResult
                   return $ pushInput c2 result

amplify :: [Computer] -> Maybe Computer
amplify (amp:amps) = do
                     final <- foldM runAndLink (pushInput amp 0) amps
                     run final
...
main = do
    ...
    let ampSets = map (zipWith pushInput $ replicate 5 comp) $ permute [0..4]
    print $ take 1 $ L.sortBy (flip compare) $ concat $ map ((take 1) . output) $ catMaybes $ map amplify ampSets
